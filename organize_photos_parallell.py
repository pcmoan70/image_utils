...
def get_exif_and_hash(p: Path, min_bytes: int) -> Optional[dict]:
    """
    Reads file once, extracts EXIF, calculates hash, returns all metadata needed for processing.
    Returns None if file is too small or not an image.
    """
    try:
        size = p.stat().st_size
        if size < min_bytes:
...
def extract_meta_and_date_source(p: Path, exif: dict) -> Tuple[Optional[datetime], str, str, str, Optional[str]]:
    """
    Returns: (dt or None, date_source, make, model, raw_exif_dt_str)
    date_source in {"exif","filename","undated"}.
    """
    # exif is now passed in from parallel worker, not read here
    # This function expects exif to be provided externally in the new workflow
...
def main():
    ...
    candidates = [p for p in src.rglob("*") if p.is_file() and p.suffix.lower() in IMG_EXTS]
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        for res in tqdm.tqdm(executor.map(get_exif_and_hash, candidates, [a.min_bytes]*total), total=total, desc="Processing files"):
            results.append(res)
    ...
    for res in results:
        ...
        name = res["name"]
        try:
            dt, date_source, make, model, raw_exif_dt = extract_meta_and_date_source(p, exif)
        ...
        target = unique_target(dst_dir, name)
